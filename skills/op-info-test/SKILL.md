...
6. If the user explicitly asks for remote validation or remote evidence, use [workflow/remote_validation_adapter.md](workflow/remote_validation_adapter.md) only as an execution adapter for the same validation loop.
7. If you need fast iteration on one newly added operator, use [workflow/patch_out_old_tests.md](workflow/patch_out_old_tests.md) as a temporary isolation tactic and remove that patch before final full validation.
8. Write the final summary and finish only after the required matrix and any required consistency step pass, or after the blocking reason is explicitly classified.
...
- Write `op_info_test_{op_name}_summary.md` in the working directory and do not add it to git. Include covered scenarios, uncovered scenarios, blocking reasons, remote job evidence, and MS/PTA consistency evidence when applicable.
