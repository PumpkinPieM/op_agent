...
KERNEL_SOURCE = Path(__file__).with_name("npu_fusion_attention_grad.cc")
_CUSTOM_OPS = None
...
def _ops():
    ...
    if _CUSTOM_OPS is None:
        ...
        try:
            _CUSTOM_OPS = ms.ops.CustomOpBuilder("custom_ops_npu_fusion_attention_grad_test", [str(KERNEL_SOURCE)], backend="Ascend").load()
...
def npu_fusion_attention_grad(query, key, value, dy, head_num, input_layout, *, pse=None, padding_mask=None, atten_mask=None, softmax_max=None, softmax_sum=None, softmax_in=None, attention_in=None, scale_value=1., keep_prob=1., pre_tockens=2147483647, next_tockens=2147483647, inner_precise=0, seed=0, offset=0, numels=0, prefix=None, actual_seq_qlen=None, actual_seq_kvlen=None, sparse_mode=0, gen_mask_parallel=True, sync=False, softmax_layout="", sink=None):
    return _ops().npu_fusion_attention_grad(query, key, value, dy, head_num, input_layout, pse, padding_mask, atten_mask, softmax_max, softmax_sum, softmax_in, attention_in, scale_value, keep_prob, pre_tockens, next_tockens, inner_precise, seed, offset, numels, prefix, actual_seq_qlen, actual_seq_kvlen, sparse_mode, gen_mask_parallel, sync, softmax_layout, sink)


def test_npu_fusion_attention_grad_matches_torch_npu():
    if not hasattr(torch_npu, "npu_fusion_attention_grad"):
        pytest.skip("benchmark torch_npu API is unavailable on this host")
    try:
        expected = torch_npu.npu_fusion_attention_grad(_torch_f16((2, 2)), _torch_f16((2, 2)), _torch_f16((2, 2)), _torch_f16((2, 2)), 0, "BSH", pse=None, padding_mask=None, atten_mask=None, softmax_max=None, softmax_sum=None, softmax_in=None, attention_in=None, scale_value=1., keep_prob=1., pre_tockens=2147483647, next_tockens=2147483647, inner_precise=0, seed=0, offset=0, numels=0, prefix=[2], actual_seq_qlen=[2], actual_seq_kvlen=[2], sparse_mode=0, gen_mask_parallel=True, sync=False, softmax_layout="", sink=None)
        actual = npu_fusion_attention_grad(_ms_f16((2, 2)), _ms_f16((2, 2)), _ms_f16((2, 2)), _ms_f16((2, 2)), 0, "BSH", pse=None, padding_mask=None, atten_mask=None, softmax_max=None, softmax_sum=None, softmax_in=None, attention_in=None, scale_value=1., keep_prob=1., pre_tockens=2147483647, next_tockens=2147483647, inner_precise=0, seed=0, offset=0, numels=0, prefix=[2], actual_seq_qlen=[2], actual_seq_kvlen=[2], sparse_mode=0, gen_mask_parallel=True, sync=False, softmax_layout="", sink=None)
