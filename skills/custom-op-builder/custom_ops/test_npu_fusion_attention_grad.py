...
def _ops():
    ...
    if _CUSTOM_OPS is None:
        ...
        _CUSTOM_OPS = ms.ops.CustomOpBuilder(
            "custom_ops_npu_fusion_attention_grad_test", [str(KERNEL_SOURCE)], backend="Ascend").load()
    return _CUSTOM_OPS
...
def npu_fusion_attention_grad(query, key, value, dy, head_num, input_layout, *, pse=None, padding_mask=None,
    ...
    return tuple(_ops().npu_fusion_attention_grad(
        ...
        numels, prefix, actual_seq_qlen, actual_seq_kvlen, sparse_mode, gen_mask_parallel, sync, softmax_layout, sink))
...
def test_npu_fusion_attention_grad_matches_torch_npu(shape, head_num, input_layout):
    ...
    attention, softmax_max, softmax_sum, softmax_out = torch_npu.npu_fusion_attention(
        torch_q, torch_k, torch_v, head_num, input_layout, None, None, None, 1., 1.,
        2147483647, 2147483647, 0, None, None, None, 0, True, False, "", None, None, 0, 0)[:4]
    expected = torch_npu.npu_fusion_attention_grad(
