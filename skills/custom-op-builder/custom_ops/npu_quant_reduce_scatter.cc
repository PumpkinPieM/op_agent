...
ms::Tensor npu_quant_reduce_scatter(const ms::Tensor & x, const ms::Tensor & scales, const std::string & hcom, int64_t world_size, const std::optional<std::string> & reduce_op_opt, const std::optional<int64_t> & output_dtype_opt, const std::optional<int64_t> & x_dtype_opt, const std::optional<int64_t> & scales_dtype_opt) {
  auto shape=x.shape(); if(!shape.empty()) shape[0]=std::max<int64_t>(1,shape[0]/std::max<int64_t>(1,world_size)); auto out=ms::Tensor(DTypeFromOptional(output_dtype_opt.value_or(-1), ms::TypeId::kNumberTypeInt8), shape); auto reduce_op=reduce_op_opt.value_or("sum");
  auto runner = std::make_shared<ms::pynative::AclnnOpRunner>("QuantReduceScatter");
  runner->SetLaunchFunc(LAUNCH_ACLNN_FUNC(aclnnQuantReduceScatter, x, scales, hcom, world_size, reduce_op, output_dtype_opt.value_or(-1), x_dtype_opt.value_or(-1), scales_dtype_opt.value_or(-1), out));
  runner->Run({x,scales}, {out});
  return out;
}

PYBIND11_MODULE(MS_EXTENSION_NAME, m) {
  m.def("npu_quant_reduce_scatter", PYBOOST_CALLER(1, custom::npu_quant_reduce_scatter));
}
}  // namespace custom
