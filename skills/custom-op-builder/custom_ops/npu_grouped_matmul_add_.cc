...
std::vector<ms::Tensor> npu_grouped_matmul_add_(const ms::Tensor &self, const ms::Tensor &x, const ms::Tensor &weight, const ms::Tensor &group_list, bool transpose_x = true, bool transpose_weight = false, int64_t group_type = 2, const std::optional<int64_t> &group_list_type_opt = std::nullopt) {
...
PYBIND11_MODULE(MS_EXTENSION_NAME, m) {
  m.def("npu_grouped_matmul_add_", PYBOOST_CALLER(1, custom::npu_grouped_matmul_add_));
}
}  // namespace custom
