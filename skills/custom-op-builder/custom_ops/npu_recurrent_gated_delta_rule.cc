...
std::vector<ms::Tensor> npu_recurrent_gated_delta_rule(const ms::Tensor &query, const ms::Tensor &key, const ms::Tensor &value, const ms::Tensor &beta, const ms::Tensor &g, const std::optional<ms::Tensor> &initial_state_opt = std::nullopt, bool output_final_state = false) {
...
PYBIND11_MODULE(MS_EXTENSION_NAME, m) {
  m.def("npu_recurrent_gated_delta_rule", PYBOOST_CALLER(2, custom::npu_recurrent_gated_delta_rule));
}
}  // namespace custom
