...
ms::Tensor npu_quant_matmul_gelu(const ms::Tensor & x1, const ms::Tensor & x2, const ms::Tensor & x1_scale, const ms::Tensor & x2_scale, const std::optional<ms::Tensor> & bias_opt, const std::optional<std::string> & approximate_opt) {
  auto out=ms::Tensor(ms::TypeId::kNumberTypeFloat16, MatmulShape(x1,x2)); auto bias=bias_opt.value_or(ms::Tensor()); auto approximate=approximate_opt.value_or("gelu_erf");
  auto runner = std::make_shared<ms::pynative::AclnnOpRunner>("FusedQuantMatmul");
  runner->SetLaunchFunc(LAUNCH_ACLNN_FUNC(aclnnFusedQuantMatmul, x1, x2, x1_scale, x2_scale, bias_opt, approximate, out));
