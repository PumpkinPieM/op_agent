...
ms::Tensor npu_quant_matmul(const ms::Tensor & x1, const ms::Tensor & x2, const ms::Tensor & scale, const std::optional<ms::Tensor> & offset_opt, const std::optional<ms::Tensor> & pertoken_scale_opt, const std::optional<ms::Tensor> & bias_opt, const std::optional<int64_t> & output_dtype_opt, const std::optional<int64_t> & x1_dtype_opt, const std::optional<int64_t> & x2_dtype_opt, const std::optional<int64_t> & pertoken_scale_dtype_opt, const std::optional<int64_t> & scale_dtype_opt, const std::optional<std::vector<int64_t>> & group_sizes_opt, const std::optional<ms::Tensor> & y_scale_opt) {
  auto out=ms::Tensor(DTypeFromOptional(output_dtype_opt.value_or(-1), ms::TypeId::kNumberTypeFloat16), MatmulShape(x1,x2)); auto offset=offset_opt.value_or(ms::Tensor()); auto pertoken_scale=pertoken_scale_opt.value_or(ms::Tensor()); auto bias=bias_opt.value_or(ms::Tensor()); auto y_scale=y_scale_opt.value_or(ms::Tensor()); auto group_sizes=std::make_pair(group_sizes_opt.value_or(std::vector<int64_t>{}), true);
  auto runner = std::make_shared<ms::pynative::AclnnOpRunner>("QuantMatmulV5");
  runner->SetLaunchFunc(LAUNCH_ACLNN_FUNC(aclnnQuantMatmulV5, x1, x2, scale, offset_opt, pertoken_scale_opt, bias_opt, output_dtype_opt.value_or(-1), x1_dtype_opt.value_or(-1), x2_dtype_opt.value_or(-1), pertoken_scale_dtype_opt.value_or(-1), scale_dtype_opt.value_or(-1), group_sizes, y_scale_opt, out));
  runner->Run({x1,x2,scale,offset,pertoken_scale,bias,y_scale}, {out});
  return out;
}
