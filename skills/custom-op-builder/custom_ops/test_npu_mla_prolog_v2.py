...
_custom_ops = ms.ops.CustomOpBuilder("custom_ops_npu_mla_prolog_v2_test", [str(KERNEL_SOURCE)], backend="Ascend").load()
...
def _case():
    ...
    cache_index = np.zeros((2,), dtype=np.int32)
    ...
    cache_mode_opt = None
    return (_pta(token_x), _pta(weight_dq), _pta(weight_uq_qr), _pta(weight_uk), _pta(weight_dkv_kr), _pta(rmsnorm_gamma_cq), _pta(rmsnorm_gamma_ckv), _pta(rope_sin), _pta(rope_cos), _pta(cache_index), _pta(kv_cache), _pta(kr_cache), _pta(dequant_scale_x_opt), _pta(dequant_scale_w_dq_opt), _pta(dequant_scale_w_uq_qr_opt), _pta(dequant_scale_w_dkv_kr_opt), _pta(quant_scale_ckv_opt), _pta(quant_scale_ckr_opt), _pta(smooth_scales_cq_opt), rmsnorm_epsilon_cq, rmsnorm_epsilon_ckv, cache_mode_opt), (_ms(token_x), _ms(weight_dq), _ms(weight_uq_qr), _ms(weight_uk), _ms(weight_dkv_kr), _ms(rmsnorm_gamma_cq), _ms(rmsnorm_gamma_ckv), _ms(rope_sin), _ms(rope_cos), _ms(cache_index), _ms(kv_cache), _ms(kr_cache), _ms(dequant_scale_x_opt), _ms(dequant_scale_w_dq_opt), _ms(dequant_scale_w_uq_qr_opt), _ms(dequant_scale_w_dkv_kr_opt), _ms(quant_scale_ckv_opt), _ms(quant_scale_ckr_opt), _ms(smooth_scales_cq_opt), rmsnorm_epsilon_cq, rmsnorm_epsilon_ckv, cache_mode_opt)
def _torch_reference(torch_args):
    required_count = 12
    keyword_names = ['dequant_scale_x', 'dequant_scale_w_dq', 'dequant_scale_w_uq_qr', 'dequant_scale_w_dkv_kr', 'quant_scale_ckv', 'quant_scale_ckr', 'smooth_scales_cq', 'rmsnorm_epsilon_cq', 'rmsnorm_epsilon_ckv', 'cache_mode']
    ...
    return torch_npu.npu_mla_prolog_v2(*torch_args[:required_count], **kwargs)

def test_npu_mla_prolog_v2_against_torch_npu_benchmark():
    assert hasattr(torch_npu, "npu_mla_prolog_v2")
