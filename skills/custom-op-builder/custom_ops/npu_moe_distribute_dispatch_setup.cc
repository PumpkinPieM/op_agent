...
std::vector<ms::Tensor> npu_moe_distribute_dispatch_setup(const ms::Tensor & x, const ms::Tensor & expert_ids, const std::string & group_ep, int64_t ep_world_size, int64_t ep_rank_id, int64_t moe_expert_num, const std::optional<ms::Tensor> & scales_opt, const std::optional<ms::Tensor> & x_active_mask_opt, int64_t expert_shard_type, int64_t shared_expert_num, int64_t shared_expert_rank_num, int64_t quant_mode, int64_t global_bs, int64_t comm_type, const std::string & comm_alg, const std::optional<int64_t> & y_dtype_opt) {
  auto expand_x=ms::Tensor(DTypeFromOptional(y_dtype_opt.value_or(-1),x.data_type()), x.shape()); auto dynamic_scales=ms::Tensor(ms::TypeId::kNumberTypeFloat32, std::vector<int64_t>{1}); auto assist=ms::Tensor(ms::TypeId::kNumberTypeInt32, std::vector<int64_t>{1}); auto scales=scales_opt.value_or(ms::Tensor()); auto x_active_mask=x_active_mask_opt.value_or(ms::Tensor());
  auto runner = std::make_shared<ms::pynative::AclnnOpRunner>("MoeDistributeDispatchSetup");
  runner->SetLaunchFunc(LAUNCH_ACLNN_FUNC(aclnnMoeDistributeDispatchSetup, x, expert_ids, group_ep, ep_world_size, ep_rank_id, moe_expert_num, scales_opt, x_active_mask_opt, expert_shard_type, shared_expert_num, shared_expert_rank_num, quant_mode, global_bs, comm_type, comm_alg, y_dtype_opt.value_or(-1), expand_x, dynamic_scales, assist));
  runner->Run({x,expert_ids,scales,x_active_mask}, {expand_x,dynamic_scales,assist});
  return {expand_x,dynamic_scales,assist};
}

PYBIND11_MODULE(MS_EXTENSION_NAME, m) {
  m.def("npu_moe_distribute_dispatch_setup", PYBOOST_CALLER(3, custom::npu_moe_distribute_dispatch_setup));
}
}  // namespace custom
