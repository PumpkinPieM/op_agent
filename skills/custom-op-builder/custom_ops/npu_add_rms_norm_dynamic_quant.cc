...
std::vector<ms::Tensor> npu_add_rms_norm_dynamic_quant(
  ...
  auto y2 = ms::Tensor(ms::TypeId::kNumberTypeInt8, output_mask.size() > 1 && output_mask[1] ? x1.shape() : std::vector<int64_t>{0});
  auto x_out = ms::Tensor(x1.data_type(), x1.shape());
  auto scale1 = ms::Tensor(ms::TypeId::kNumberTypeFloat32, ReduceLastDim(x1));
  auto scale2 = ms::Tensor(ms::TypeId::kNumberTypeFloat32, output_mask.size() > 1 && output_mask[1] ? ReduceLastDim(x1) : std::vector<int64_t>{0});
  ...
  runner->SetLaunchFunc(LAUNCH_ACLNN_FUNC(aclnnAddRmsNormDynamicQuantV2, x1, x2, gamma, smooth_scale1_opt,
                                          smooth_scale2_opt, beta_opt, epsilon, output_mask, y1, y2, x_out, scale1, scale2));
  runner->Run({x1, x2, gamma, smooth_scale1, smooth_scale2, beta}, {y1, y2, x_out, scale1, scale2});
  return {y1, y2, x_out, scale1, scale2};
}
PYBIND11_MODULE(MS_EXTENSION_NAME, m) { m.def("npu_add_rms_norm_dynamic_quant", PYBOOST_CALLER(5, custom::npu_add_rms_norm_dynamic_quant)); }
