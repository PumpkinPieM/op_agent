...
std::vector<ms::Tensor> npu_fusion_attention_grad(
    ...
    int64_t seed, int64_t offset, int64_t numels, const std::optional<std::vector<int64_t>> &prefix_opt,
  ...
  runner->Run({query, key, value, dy, pse_value, padding_mask_value, atten_mask_value, softmax_max_value,
               softmax_sum_value, softmax_in_value, attention_in_value},
              {dq, dk, dv, dpse});
  return {dq, dk, dv, dpse};
}

PYBIND11_MODULE(MS_EXTENSION_NAME, m) {
  m.def("npu_fusion_attention_grad", PYBOOST_CALLER(4, custom::npu_fusion_attention_grad));
}
}  // namespace custom
