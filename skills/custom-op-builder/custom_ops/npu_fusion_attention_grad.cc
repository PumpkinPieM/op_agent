...
std::vector<ms::Tensor> npu_fusion_attention_grad(const ms::Tensor &query, const ms::Tensor &key, const ms::Tensor &value, const ms::Tensor &dy, int64_t head_num, const std::string &input_layout, const std::optional<ms::Tensor> &pse_opt, const std::optional<ms::Tensor> &padding_mask_opt, const std::optional<ms::Tensor> &atten_mask_opt, const std::optional<ms::Tensor> &softmax_max_opt, const std::optional<ms::Tensor> &softmax_sum_opt, const std::optional<ms::Tensor> &softmax_in_opt, const std::optional<ms::Tensor> &attention_in_opt, double scale_value, double keep_prob, int64_t pre_tockens, int64_t next_tockens, int64_t inner_precise, int64_t seed, int64_t offset, int64_t numels, const std::optional<std::vector<int64_t>> &prefix_opt, const std::optional<std::vector<int64_t>> &actual_seq_qlen_opt, const std::optional<std::vector<int64_t>> &actual_seq_kvlen_opt, int64_t sparse_mode, bool gen_mask_parallel, bool sync, const std::string &softmax_layout, const std::optional<ms::Tensor> &sink_opt) {
  ...
  runner->SetLaunchFunc(LAUNCH_ACLNN_FUNC(aclnnFlashAttentionScoreGrad, query, key, value, dy, head_num, input_layout, pse_opt, padding_mask_opt, atten_mask_opt, softmax_max_opt, softmax_sum_opt, softmax_in_opt, attention_in_opt, scale_value, keep_prob, pre_tockens, next_tockens, inner_precise, seed, offset, numels, prefix, actual_seq_qlen, actual_seq_kvlen, sparse_mode, gen_mask_parallel, sync, softmax_layout, sink_opt, out0, out1, out2, out3, out4));
  runner->Run({query, key, value, dy, pse_value, padding_mask_value, atten_mask_value, softmax_max_value, softmax_sum_value, softmax_in_value, attention_in_value, sink_value}, {out0, out1, out2, out3, out4});
  return {out0, out1, out2, out3, out4};
}

PYBIND11_MODULE(MS_EXTENSION_NAME, m) {
  m.def("npu_fusion_attention_grad", PYBOOST_CALLER(5, custom::npu_fusion_attention_grad));
}
}  // namespace custom
