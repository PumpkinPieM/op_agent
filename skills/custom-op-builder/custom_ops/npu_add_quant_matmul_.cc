...
std::vector<ms::Tensor> npu_add_quant_matmul_(const ms::Tensor &self, const ms::Tensor &x1, const ms::Tensor &x2, const ms::Tensor &x2_scale, const ms::Tensor &group_list, const std::optional<ms::Tensor> &x1_scale_opt = std::nullopt, const std::optional<int64_t> &group_list_type_opt = std::nullopt, const std::optional<std::vector<int64_t>> &group_sizes_opt = std::nullopt, const std::optional<int64_t> &x1_dtype_opt = std::nullopt, const std::optional<int64_t> &x2_dtype_opt = std::nullopt, const std::optional<int64_t> &x1_scale_dtype_opt = std::nullopt, const std::optional<int64_t> &x2_scale_dtype_opt = std::nullopt) {
...
PYBIND11_MODULE(MS_EXTENSION_NAME, m) {
  m.def("npu_add_quant_matmul_", PYBOOST_CALLER(1, custom::npu_add_quant_matmul_));
}
}  // namespace custom
