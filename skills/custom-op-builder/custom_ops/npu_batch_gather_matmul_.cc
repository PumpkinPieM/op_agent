...
std::vector<ms::Tensor> npu_batch_gather_matmul_(const ms::Tensor &self, const ms::Tensor &x, const ms::Tensor &weight_b, const ms::Tensor &indices, const std::optional<ms::Tensor> &weight_a_opt, int64_t layer_idx, double scale, int64_t y_offset, int64_t y_slice_size) {
...
PYBIND11_MODULE(MS_EXTENSION_NAME, m) {
  m.def("npu_batch_gather_matmul_", PYBOOST_CALLER(1, custom::npu_batch_gather_matmul_));
}
}  // namespace custom
