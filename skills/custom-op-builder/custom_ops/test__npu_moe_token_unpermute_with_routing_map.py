...
def _ops():
    ...
    if _CUSTOM_OPS is None:
        ...
        try:
            _CUSTOM_OPS = ms.ops.CustomOpBuilder("custom_ops__npu_moe_token_unpermute_with_routing_map_test", [str(KERNEL_SOURCE)], backend="Ascend").load()
...
def _npu_moe_token_unpermute_with_routing_map(permuted_tokens, sorted_indices, restore_shape, *, probs=None, routing_map=None, drop_and_pad=False):
    return _ops()._npu_moe_token_unpermute_with_routing_map(permuted_tokens, sorted_indices, restore_shape, probs, routing_map, drop_and_pad)


CASES = [
 {"id":"drop_and_pad_true_with_probs","torch":lambda: torch_npu._npu_moe_token_unpermute_with_routing_map(_torch_tensor(np.ones((4,8),np.float16)),_torch_tensor(np.arange(4,dtype=np.int32)),[4,8],probs=_torch_tensor(np.ones((4,),np.float16)),routing_map=_torch_tensor(np.eye(4,dtype=np.int8)),drop_and_pad=True),"ms":lambda: _npu_moe_token_unpermute_with_routing_map(_ms_tensor(np.ones((4,8),np.float16)),_ms_tensor(np.arange(4,dtype=np.int32)),[4,8],probs=_ms_tensor(np.ones((4,),np.float16)),routing_map=_ms_tensor(np.eye(4,dtype=np.int8)),drop_and_pad=True)},
]
