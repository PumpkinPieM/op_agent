...
KERNEL_SOURCE = Path(__file__).with_name("npu_quant_matmul_dequant.cc")

torch.npu.set_device(DEVICE_ID)
torch.npu.set_compile_mode(jit_compile=False)
context.set_context(device_target="Ascend", device_id=DEVICE_ID)
context.set_context(mode=ms.PYNATIVE_MODE, deterministic="ON", pynative_synchronize=False)
_custom_ops = ms.ops.CustomOpBuilder("custom_ops_npu_quant_matmul_dequant_test", [str(KERNEL_SOURCE)], backend="Ascend").load()
def npu_quant_matmul_dequant(*args):
    return _custom_ops.npu_quant_matmul_dequant(*args)
...
def _case():
    ...
    quant_mode_opt = None
    return (_pta(x), _pta(quantized_weight), _pta(weight_scale), _pta(bias_opt), _pta(x_scale_opt), _pta(x_offset_opt), _pta(smooth_scale_opt), quant_mode_opt), (_ms(x), _ms(quantized_weight), _ms(weight_scale), _ms(bias_opt), _ms(x_scale_opt), _ms(x_offset_opt), _ms(smooth_scale_opt), quant_mode_opt)
def _torch_reference(torch_args):
    ...
    return torch_npu.npu_quant_matmul_dequant(*torch_args[:required_count], **kwargs)
