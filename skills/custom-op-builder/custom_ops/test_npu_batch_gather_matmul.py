...
def _ops():
    ...
    if _CUSTOM_OPS is None:
        ...
        _CUSTOM_OPS = ms.ops.CustomOpBuilder(
            "custom_ops_npu_batch_gather_matmul_test_v5",
            [str(KERNEL_SOURCE)],
            backend="Ascend",
        ).load()
    return _CUSTOM_OPS
...
def npu_batch_gather_matmul(self, x, weight_b, indices, weight_a=None, layer_idx=0, scale=1e-3, y_offset=0,
                            y_slice_size=-1):
    return _ops().npu_batch_gather_matmul(self, x, weight_b, indices, weight_a, layer_idx, scale, y_offset,
                                          y_slice_size)
...
def test_npu_batch_gather_matmul_matches_torch_npu(case_fn):
    _run_case(case_fn())
