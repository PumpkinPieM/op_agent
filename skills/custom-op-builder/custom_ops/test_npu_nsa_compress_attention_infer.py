...
_custom_ops = ms.ops.CustomOpBuilder("custom_ops_npu_nsa_compress_attention_infer_test", [str(KERNEL_SOURCE)], backend="Ascend").load()
...
def _case():
    ...
    actual_cmp_seq_kvlen_opt = None
    actual_sel_seq_kvlen_opt = None
    return (_pta(query), _pta(key), _pta(value), scale_value, head_num, key_value_head_num, select_block_size, select_block_count, page_block_size, compress_block_size, compress_stride, layout_opt, _pta(atten_mask_opt), _pta(block_table_opt), _pta(topk_mask_opt), actual_seq_qlen_opt, actual_cmp_seq_kvlen_opt, actual_sel_seq_kvlen_opt), (_ms(query), _ms(key), _ms(value), scale_value, head_num, key_value_head_num, select_block_size, select_block_count, page_block_size, compress_block_size, compress_stride, layout_opt, _ms(atten_mask_opt), _ms(block_table_opt), _ms(topk_mask_opt), actual_seq_qlen_opt, actual_cmp_seq_kvlen_opt, actual_sel_seq_kvlen_opt)
def _torch_reference(torch_args):
    required_count = 11
    keyword_names = ['layout', 'atten_mask', 'block_table', 'topk_mask', 'actual_seq_qlen', 'actual_cmp_seq_kvlen', 'actual_sel_seq_kvlen']
    ...
    return torch_npu.npu_nsa_compress_attention_infer(*torch_args[:required_count], **kwargs)

def test_npu_nsa_compress_attention_infer_against_torch_npu_benchmark():
    assert hasattr(torch_npu, "npu_nsa_compress_attention_infer")
