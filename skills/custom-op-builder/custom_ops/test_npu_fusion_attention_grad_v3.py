...
def _ops():
    ...
    if _CUSTOM_OPS is None:
        ...
        _CUSTOM_OPS = ms.ops.CustomOpBuilder(
            "custom_ops_npu_fusion_attention_grad_v3_test", [str(KERNEL_SOURCE)], backend="Ascend").load()
    return _CUSTOM_OPS
...
def npu_fusion_attention_grad_v3(query, key, value, dy, head_num, input_layout, *, pse=None, padding_mask=None,
    ...
    return tuple(_ops().npu_fusion_attention_grad_v3(
        query, key, value, dy, head_num, input_layout, pse, padding_mask, atten_mask, softmax_max, softmax_sum,
        softmax_in, attention_in, scale_value, keep_prob, pre_tockens, next_tockens, inner_precise, seed, offset,
...
def test_npu_fusion_attention_grad_v3_matches_torch_npu(shape, head_num, input_layout):
    ...
    attention, softmax_max, softmax_sum, softmax_out = torch_npu.npu_fusion_attention_v3(
        torch_q, torch_k, torch_v, head_num, input_layout, None, None, None, 1., 1.,
        2147483647, 2147483647, 0, None, None, None, 0, True, False, "", None)[:4]
    expected = torch_npu.npu_fusion_attention_grad_v3(
        ...
        attention_in=attention, scale_value=1., keep_prob=1., pre_tockens=2147483647,
        next_tockens=2147483647, inner_precise=0, seed=None, offset=None, prefix=None,
    ...
    actual = npu_fusion_attention_grad_v3(
        ...
        attention_in=_ms_from_torch(attention), scale_value=1., keep_prob=1., pre_tockens=2147483647,
        next_tockens=2147483647, inner_precise=0, seed=None, offset=None, prefix=None,
        actual_seq_qlen=None, actual_seq_kvlen=None, sparse_mode=0, gen_mask_parallel=True, sync=False,
        softmax_layout="", sink=None)
