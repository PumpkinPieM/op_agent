...
std::vector<ms::Tensor> npu_gather_pa_kv_cache_functional(const ms::Tensor &key_cache, const ms::Tensor &value_cache, const ms::Tensor &block_tables, const ms::Tensor &seq_lens, const ms::Tensor &key, const ms::Tensor &value, const std::optional<ms::Tensor> &seq_offset_opt = std::nullopt, bool is_seq_lens_cumsum = false) {
...
PYBIND11_MODULE(MS_EXTENSION_NAME, m) {
  m.def("npu_gather_pa_kv_cache_functional", PYBOOST_CALLER(2, custom::npu_gather_pa_kv_cache_functional));
}
}  // namespace custom
