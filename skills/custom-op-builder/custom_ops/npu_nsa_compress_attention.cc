...
std::vector<ms::Tensor> npu_nsa_compress_attention(const ms::Tensor & query, const ms::Tensor & key, const ms::Tensor & value, double scale_value, int64_t head_num, int64_t compress_block_size, int64_t compress_stride, int64_t select_block_size, int64_t select_block_count, const std::optional<ms::Tensor> & topk_mask_opt, const std::optional<ms::Tensor> & atten_mask_opt, const std::optional<std::vector<int64_t>> & actual_seq_qlen_opt, const std::optional<std::vector<int64_t>> & actual_cmp_seq_kvlen_opt, const std::optional<std::vector<int64_t>> & actual_sel_seq_kvlen_opt) {
  auto attention_out=ms::Tensor(query.data_type(), query.shape()); auto topk_indices=ms::Tensor(ms::TypeId::kNumberTypeInt32, std::vector<int64_t>{query.shape()[0], head_num, select_block_count}); auto softmax_max=ms::Tensor(ms::TypeId::kNumberTypeFloat32, std::vector<int64_t>{1}); auto softmax_sum=ms::Tensor(ms::TypeId::kNumberTypeFloat32, std::vector<int64_t>{1}); auto topk_mask=topk_mask_opt.value_or(ms::Tensor()); auto atten_mask=atten_mask_opt.value_or(ms::Tensor()); auto actual_seq_qlen=std::make_pair(actual_seq_qlen_opt.value_or(std::vector<int64_t>{}), true); auto actual_cmp_seq_kvlen=std::make_pair(actual_cmp_seq_kvlen_opt.value_or(std::vector<int64_t>{}), true); auto actual_sel_seq_kvlen=std::make_pair(actual_sel_seq_kvlen_opt.value_or(std::vector<int64_t>{}), true);
  auto runner = std::make_shared<ms::pynative::AclnnOpRunner>("NsaCompressAttention");
  runner->SetLaunchFunc(LAUNCH_ACLNN_FUNC(aclnnNsaCompressAttention, query, key, value, scale_value, head_num, compress_block_size, compress_stride, select_block_size, select_block_count, topk_mask_opt, atten_mask_opt, actual_seq_qlen, actual_cmp_seq_kvlen, actual_sel_seq_kvlen, attention_out, topk_indices, softmax_max, softmax_sum));
  runner->Run({query,key,value,topk_mask,atten_mask}, {attention_out,topk_indices,softmax_max,softmax_sum});
