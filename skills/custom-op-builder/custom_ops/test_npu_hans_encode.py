...
def _ops():
    ...
    if _CUSTOM_OPS is None:
        ...
        try:
            _CUSTOM_OPS = ms.ops.CustomOpBuilder("custom_ops_npu_hans_encode_test", [str(KERNEL_SOURCE)], backend="Ascend").load()
...
def npu_hans_encode(x, pdf, reshuff=False):
    return _ops().npu_hans_encode(x, pdf, reshuff)


CASES = [{"id":"reshuff_false","torch":lambda: torch_npu.npu_hans_encode(_torch_tensor(np.zeros((8,),np.int32)),_torch_tensor(np.ones((8,),np.int32)),reshuff=False),"ms":lambda: npu_hans_encode(_ms_tensor(np.zeros((8,),np.int32)),_ms_tensor(np.ones((8,),np.int32)),reshuff=False)},{"id":"reshuff_true","torch":lambda: torch_npu.npu_hans_encode(_torch_tensor(np.zeros((8,),np.int32)),_torch_tensor(np.ones((8,),np.int32)),reshuff=True),"ms":lambda: npu_hans_encode(_ms_tensor(np.zeros((8,),np.int32)),_ms_tensor(np.ones((8,),np.int32)),reshuff=True)}]


@pytest.mark.parametrize("case", CASES, ids=lambda c: c["id"])
def test_npu_hans_encode_matches_torch_npu(case):
    try:
        expected = case["torch"]()
        actual = case["ms"]()
