...
ms::Tensor npu_quant_scatter(const ms::Tensor & self, const ms::Tensor & indices, const ms::Tensor & updates, const ms::Tensor & quant_scales, const std::optional<ms::Tensor> & quant_zero_points_opt, int64_t axis, int64_t quant_axis, const std::string & reduce, const std::optional<int64_t> & dst_type_opt, const std::optional<std::string> & round_mode_opt) {
...
PYBIND11_MODULE(MS_EXTENSION_NAME, m) {
  m.def("npu_quant_scatter", PYBOOST_CALLER(1, custom::npu_quant_scatter));
}
}  // namespace custom
