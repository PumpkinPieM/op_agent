...
KERNEL_SOURCE = Path(__file__).with_name("npu_moe_update_expert.cc")
_CUSTOM_OPS = None
...
def _ops():
    ...
    if _CUSTOM_OPS is None:
        ...
        _CUSTOM_OPS = ms.ops.CustomOpBuilder("custom_ops_npu_moe_update_expert_test", [str(KERNEL_SOURCE)], backend="Ascend").load()
    return _CUSTOM_OPS
...
def npu_moe_update_expert(expert_ids, expert_scales, expert_num):
    return _ops().npu_moe_update_expert(expert_ids, expert_scales, expert_num)


CASES = [{"id":"int32_fp32","torch":lambda: torch_npu.npu_moe_update_expert(_torch_tensor(np.zeros((4,),np.int32)),_torch_tensor(np.ones((4,),np.float32)),2),"ms":lambda: npu_moe_update_expert(_ms_tensor(np.zeros((4,),np.int32)),_ms_tensor(np.ones((4,),np.float32)),2)}]


@pytest.mark.parametrize("case", CASES, ids=lambda c: c["id"])
def test_npu_moe_update_expert_matches_torch_npu(case):
    expected = case["torch"]()
    actual = case["ms"]()
