...
KERNEL_SOURCE = Path(__file__).with_name("_npu_dropout.cc")
_CUSTOM_OPS = None
...
def _ops():
    ...
    if _CUSTOM_OPS is None:
        ...
        try:
            _CUSTOM_OPS = ms.ops.CustomOpBuilder("custom_ops__npu_dropout_test", [str(KERNEL_SOURCE)], backend="Ascend").load()
...
def _npu_dropout(self, p):
    return _ops()._npu_dropout(self, p)


CASES = [
    {"id": "p0_fp16_ones", "torch": lambda: torch_npu._npu_dropout(_torch_tensor(np.ones((2, 3), np.float16)), 0.0), "ms": lambda: _npu_dropout(_ms_tensor(np.ones((2, 3), np.float16)), 0.0)},
    {"id": "p0_fp16_arange", "torch": lambda: torch_npu._npu_dropout(_torch_tensor(np.arange(6, dtype=np.float16).reshape(2, 3)), 0.0), "ms": lambda: _npu_dropout(_ms_tensor(np.arange(6, dtype=np.float16).reshape(2, 3)), 0.0)},
]


@pytest.mark.parametrize("case", CASES, ids=lambda c: c["id"])
def test_npu_dropout_matches_torch_npu(case):
    try:
        expected = case["torch"]()
        actual = case["ms"]()
