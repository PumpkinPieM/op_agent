...
def _ops():
    ...
    if _CUSTOM_OPS is None:
        ...
        try:
            _CUSTOM_OPS = ms.ops.CustomOpBuilder("custom_ops_npu_fused_floyd_attention_test", [str(KERNEL_SOURCE)], backend="Ascend").load()
...
def npu_fused_floyd_attention(query_ik, key_ij, value_ij, key_jk, value_jk, *, atten_mask=None, scale_value=1.):
    return _ops().npu_fused_floyd_attention(query_ik, key_ij, value_ij, key_jk, value_jk, atten_mask, scale_value)


def test_npu_fused_floyd_attention_matches_torch_npu():
    if not hasattr(torch_npu, "npu_fused_floyd_attention"):
        pytest.skip("benchmark torch_npu API is unavailable on this host")
    try:
        expected = torch_npu.npu_fused_floyd_attention(_torch_f16((2, 2)), _torch_f16((2, 2)), _torch_f16((2, 2)), _torch_f16((2, 2)), _torch_f16((2, 2)), atten_mask=None, scale_value=1.)
        actual = npu_fused_floyd_attention(_ms_f16((2, 2)), _ms_f16((2, 2)), _ms_f16((2, 2)), _ms_f16((2, 2)), _ms_f16((2, 2)), atten_mask=None, scale_value=1.)
