...
ms::Tensor npu_weight_quant_batchmatmul(const ms::Tensor & x, const ms::Tensor & weight, const ms::Tensor & antiquant_scale, const std::optional<ms::Tensor> & antiquant_offset_opt, const std::optional<ms::Tensor> & quant_scale_opt, const std::optional<ms::Tensor> & quant_offset_opt, const std::optional<ms::Tensor> & bias_opt, int64_t antiquant_group_size, int64_t inner_precise, const std::optional<int64_t> & weight_dtype_opt) {
  auto out=ms::Tensor(quant_scale_opt.has_value()?ms::TypeId::kNumberTypeInt8:x.data_type(), MatmulShape(x,weight)); auto antiquant_offset=antiquant_offset_opt.value_or(ms::Tensor()); auto quant_scale=quant_scale_opt.value_or(ms::Tensor()); auto quant_offset=quant_offset_opt.value_or(ms::Tensor()); auto bias=bias_opt.value_or(ms::Tensor());
  auto runner = std::make_shared<ms::pynative::AclnnOpRunner>("WeightQuantBatchMatmulV3");
  runner->SetLaunchFunc(LAUNCH_ACLNN_FUNC(aclnnWeightQuantBatchMatmulV3, x, weight, antiquant_scale, antiquant_offset_opt, quant_scale_opt, quant_offset_opt, bias_opt, antiquant_group_size, inner_precise, out));
  runner->Run({x,weight,antiquant_scale,antiquant_offset,quant_scale,quant_offset,bias}, {out});
  return out;
}

PYBIND11_MODULE(MS_EXTENSION_NAME, m) {
  m.def("npu_weight_quant_batchmatmul", PYBOOST_CALLER(1, custom::npu_weight_quant_batchmatmul));
}
}  // namespace custom
