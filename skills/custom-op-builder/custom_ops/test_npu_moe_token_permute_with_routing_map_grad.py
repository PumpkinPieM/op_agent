...
KERNEL_SOURCE = Path(__file__).with_name("npu_moe_token_permute_with_routing_map_grad.cc")
_CUSTOM_OPS = None
...
def npu_moe_token_permute_with_routing_map_grad(permuted_token_out_grad, probs_grad, sorted_indices, routing_map, experts_num, tokens_num, drop_and_pad):
    return _ops().npu_moe_token_permute_with_routing_map_grad(permuted_token_out_grad, probs_grad, sorted_indices, routing_map, experts_num, tokens_num, drop_and_pad)


CASES = [
 {"id":"drop_and_pad_true","torch":lambda: torch_npu.npu_moe_token_permute_with_routing_map_grad(_torch_tensor(np.ones((4,8),np.float16)),_torch_tensor(np.ones((4,),np.float16)),_torch_tensor(np.arange(4,dtype=np.int32)),_torch_tensor(np.eye(4,dtype=np.int8)),4,4,True),"ms":lambda: npu_moe_token_permute_with_routing_map_grad(_ms_tensor(np.ones((4,8),np.float16)),_ms_tensor(np.ones((4,),np.float16)),_ms_tensor(np.arange(4,dtype=np.int32)),_ms_tensor(np.eye(4,dtype=np.int8)),4,4,True)},
]
