...
def _ops():
    ...
    if _CUSTOM_OPS is None:
        ...
        try:
            _CUSTOM_OPS = ms.ops.CustomOpBuilder("custom_ops_npu_moe_init_routing_v2_test", [str(KERNEL_SOURCE)], backend="Ascend").load()
...
def npu_moe_init_routing_v2(x, expert_idx, *, scale=None, expanded_x_out=None, active_num=None):
    return _ops().npu_moe_init_routing_v2(x, expert_idx, scale, expanded_x_out, active_num)


CASES = [
 {"id":"default_optional","torch":lambda: torch_npu.npu_moe_init_routing_v2(_torch_tensor(np.ones((4,8),np.float16)),_torch_tensor(np.zeros((4,),np.int32))),"ms":lambda: npu_moe_init_routing_v2(_ms_tensor(np.ones((4,8),np.float16)),_ms_tensor(np.zeros((4,),np.int32)))},
 {"id":"with_scale_active_num","torch":lambda: torch_npu.npu_moe_init_routing_v2(_torch_tensor(np.ones((4,8),np.float16)),_torch_tensor(np.zeros((4,),np.int32)),scale=_torch_tensor(np.ones((4,),np.float16)),active_num=4),"ms":lambda: npu_moe_init_routing_v2(_ms_tensor(np.ones((4,8),np.float16)),_ms_tensor(np.zeros((4,),np.int32)),scale=_ms_tensor(np.ones((4,),np.float16)),active_num=4)},
]
