...
std::vector<ms::Tensor> npu_fusion_attention_grad_v3(
    ...
    const std::optional<ms::Tensor> &seed_opt, const std::optional<ms::Tensor> &offset_opt,
    const std::optional<std::vector<int64_t>> &prefix_opt, const std::optional<ms::Tensor> &actual_seq_qlen_opt,
    const std::optional<ms::Tensor> &actual_seq_kvlen_opt, int64_t sparse_mode, bool gen_mask_parallel, bool sync,
    const std::string &softmax_layout, const std::optional<ms::Tensor> &sink_opt) {
  ...
  auto q_start_idx = std::make_pair(std::vector<int64_t>{}, true);
  auto kv_start_idx = std::make_pair(std::vector<int64_t>{}, true);
  ...
  auto runner = std::make_shared<ms::pynative::AclnnOpRunner>("FlashAttentionScoreGradV3");
  if (sink_opt.has_value()) {
    runner->SetLaunchFunc(LAUNCH_ACLNN_FUNC(aclnnFlashAttentionScoreGradV3, query, key, value, dy, pse_opt,
                                            std::optional<ms::Tensor>(), padding_mask_opt, atten_mask_opt,
                                            softmax_max_opt, softmax_sum_opt, softmax_in_opt, attention_in_opt,
                                            sink_opt, prefix, q_start_idx, kv_start_idx, scale_value, keep_prob,
                                            pre_tockens, next_tockens, head_num, input_layout, inner_precise,
                                            sparse_mode, 1, dq, dk, dv, dpse, dsink));
  } else {
    runner->SetLaunchFunc(LAUNCH_ACLNN_FUNC(aclnnFlashAttentionScoreGrad, query, key, value, dy, pse_opt,
                                            std::optional<ms::Tensor>(), padding_mask_opt, atten_mask_opt,
                                            softmax_max_opt, softmax_sum_opt, softmax_in_opt, attention_in_opt, prefix,
                                            scale_value, keep_prob, pre_tockens, next_tockens, head_num, input_layout,
  ...
  runner->Run({query, key, value, dy, pse_value, padding_mask_value, atten_mask_value, softmax_max_value,
               softmax_sum_value, softmax_in_value, attention_in_value, sink_value},
              {dq, dk, dv, dpse, dsink});
