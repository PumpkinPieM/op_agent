...
bool IsQuantQueryPath(const ms::Tensor &token_x, const std::optional<ms::Tensor> &quant_scale_ckv_opt) {
  return token_x.data_type() == ms::TypeId::kNumberTypeInt8 && quant_scale_ckv_opt.has_value();
}
...
std::vector<ms::Tensor> npu_mla_prolog_v2(
    ...
    const ms::Tensor &cache_index, const ms::Tensor &kv_cache, const ms::Tensor &kr_cache,
    ...
    const std::optional<ms::Tensor> &smooth_scales_cq_opt, double rmsnorm_epsilon_cq,
    double rmsnorm_epsilon_ckv, const std::optional<std::string> &cache_mode_opt) {
  const bool quant_query_path = IsQuantQueryPath(token_x, quant_scale_ckv_opt);
  auto query = ms::Tensor(quant_query_path ? token_x.data_type() : rope_sin.data_type(), QueryShape(token_x, weight_uk));
  ...
  runner->SetLaunchFunc(LAUNCH_ACLNN_FUNC(
      aclnnMlaPrologV2WeightNz, token_x, weight_dq, weight_uq_qr, weight_uk, weight_dkv_kr, rmsnorm_gamma_cq,
      rmsnorm_gamma_ckv, rope_sin, rope_cos, cache_index, kv_cache, kr_cache, dequant_scale_x_opt,
      dequant_scale_w_dq_opt, dequant_scale_w_uq_qr_opt, dequant_scale_w_dkv_kr_opt, quant_scale_ckv_opt,
      quant_scale_ckr_opt, smooth_scales_cq_opt, rmsnorm_epsilon_cq, rmsnorm_epsilon_ckv, cache_mode, query,
      query_rope, dequant_scale_q_nope));
  runner->Run({token_x, weight_dq, weight_uq_qr, weight_uk, weight_dkv_kr, rmsnorm_gamma_cq, rmsnorm_gamma_ckv,
               rope_sin, rope_cos, cache_index, kv_cache, kr_cache, dequant_scale_x, dequant_scale_w_dq,
               dequant_scale_w_uq_qr, dequant_scale_w_dkv_kr, quant_scale_ckv, quant_scale_ckr, smooth_scales_cq},
              {query, query_rope, dequant_scale_q_nope, kv_cache, kr_cache});
  return {query, query_rope, kv_cache, kr_cache, dequant_scale_q_nope};
}
