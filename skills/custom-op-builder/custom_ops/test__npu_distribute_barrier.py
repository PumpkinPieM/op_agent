...
def _ops():
    ...
    if _CUSTOM_OPS is None:
        ...
        try:
            _CUSTOM_OPS = ms.ops.CustomOpBuilder("custom_ops__npu_distribute_barrier_test", [str(KERNEL_SOURCE)], backend="Ascend").load()
...
def _npu_distribute_barrier(x_ref, group, world_size, *, time_out=None, elastic_info=None):
    return _ops()._npu_distribute_barrier(x_ref, group, world_size, time_out, elastic_info)


CASES = [
    {"id": "default_optional", "torch": lambda: torch_npu._npu_distribute_barrier(_torch_tensor(np.ones((1,), np.float32)), "hccl_world_group", 1), "ms": lambda: _npu_distribute_barrier(_ms_tensor(np.ones((1,), np.float32)), "hccl_world_group", 1)},
    {"id": "with_optional", "torch": lambda: torch_npu._npu_distribute_barrier(_torch_tensor(np.ones((1,), np.float32)), "hccl_world_group", 1, time_out=_torch_tensor(np.array([1], np.int32)), elastic_info=_torch_tensor(np.array([0], np.int32))), "ms": lambda: _npu_distribute_barrier(_ms_tensor(np.ones((1,), np.float32)), "hccl_world_group", 1, time_out=_ms_tensor(np.array([1], np.int32)), elastic_info=_ms_tensor(np.array([0], np.int32)))},
]


@pytest.mark.parametrize("case", CASES, ids=lambda c: c["id"])
def test_npu_distribute_barrier_matches_torch_npu(case):
    try:
        expected = case["torch"]()
        actual = case["ms"]()
