...
std::vector<ms::Tensor> npu_kv_rmsnorm_rope_cache_v2_functional(const ms::Tensor &kv, const ms::Tensor &gamma, const ms::Tensor &cos, const ms::Tensor &sin, const ms::Tensor &index, const ms::Tensor &k_cache, const ms::Tensor &ckv_cache, const std::optional<ms::Tensor> &k_rope_scale_opt, const std::optional<ms::Tensor> &c_kv_scale_opt, const std::optional<ms::Tensor> &k_rope_offset_opt, const std::optional<ms::Tensor> &c_kv_offset_opt, const std::optional<ms::Tensor> &v_opt, double epsilon, const std::string &cache_mode, bool is_output_kv, const std::optional<int64_t> &k_cache_dtype_opt, const std::optional<int64_t> &ckv_cache_dtype_opt) {
  ...
  runner->SetLaunchFunc(LAUNCH_ACLNN_FUNC(aclnnKvRmsNormRopeCache, kv, gamma, cos, sin, index, k_cache, ckv_cache, k_rope_scale_opt, c_kv_scale_opt, k_rope_offset_opt, c_kv_offset_opt, v_opt, epsilon, cache_mode, is_output_kv, k_cache_dtype, ckv_cache_dtype, out0, out1, out2, out3));
...
PYBIND11_MODULE(MS_EXTENSION_NAME, m) {
  m.def("npu_kv_rmsnorm_rope_cache_v2_functional", PYBOOST_CALLER(4, custom::npu_kv_rmsnorm_rope_cache_v2_functional));
}
}  // namespace custom
