...
KERNEL_SOURCE = Path(__file__).with_name("npu_quant_reduce_scatter.cc")

torch.npu.set_device(DEVICE_ID)
torch.npu.set_compile_mode(jit_compile=False)
context.set_context(device_target="Ascend", device_id=DEVICE_ID)
context.set_context(mode=ms.PYNATIVE_MODE, deterministic="ON", pynative_synchronize=False)
_custom_ops = ms.ops.CustomOpBuilder("custom_ops_npu_quant_reduce_scatter_test", [str(KERNEL_SOURCE)], backend="Ascend").load()
def npu_quant_reduce_scatter(*args):
    return _custom_ops.npu_quant_reduce_scatter(*args)
...
def _torch_reference(torch_args):
    ...
    return torch_npu.npu_quant_reduce_scatter(*torch_args[:required_count], **kwargs)

def test_npu_quant_reduce_scatter_against_torch_npu_benchmark():
    assert hasattr(torch_npu, "npu_quant_reduce_scatter")
    torch_args, ms_args = _case()
    expected = _torch_reference(torch_args)
    actual = npu_quant_reduce_scatter(*ms_args)
