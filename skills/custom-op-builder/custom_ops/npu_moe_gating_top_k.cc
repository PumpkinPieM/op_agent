...
std::vector<ms::Tensor> npu_moe_gating_top_k(const ms::Tensor &x, int64_t k,
  ...
  auto bias = bias_opt.value_or(ms::Tensor()); auto y = ms::Tensor(x.data_type(), std::vector<int64_t>{x.shape()[0], k});
  auto idx = ms::Tensor(ms::TypeId::kNumberTypeInt32, std::vector<int64_t>{x.shape()[0], k}); auto out = ms::Tensor(ms::TypeId::kNumberTypeFloat32, x.shape());
  ...
  runner->Run({x, bias}, {y, idx, out}); return {y, idx, out};
}
PYBIND11_MODULE(MS_EXTENSION_NAME, m) { m.def("npu_moe_gating_top_k", PYBOOST_CALLER(3, custom::npu_moe_gating_top_k)); }
