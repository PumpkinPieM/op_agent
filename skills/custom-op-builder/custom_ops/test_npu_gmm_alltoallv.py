...
KERNEL_SOURCE = Path(__file__).with_name("npu_gmm_alltoallv.cc")
_CUSTOM_OPS = None
...
def _ops():
    ...
    if _CUSTOM_OPS is None:
        ...
        try:
            _CUSTOM_OPS = ms.ops.CustomOpBuilder("custom_ops_npu_gmm_alltoallv_test", [str(KERNEL_SOURCE)], backend="Ascend").load()
...
def npu_gmm_alltoallv(x1, x2, hcom, world_size, bias=None, axes=None, out_flag=True):
    return _ops().npu_gmm_alltoallv(x1, x2, hcom, world_size, bias, axes, out_flag)


CASES = [
 {"id":"out_flag_true","torch":lambda: torch_npu.npu_gmm_alltoallv(_torch_tensor(np.ones((2,8),np.float16)),_torch_tensor(np.ones((8,4),np.float16)),"hccl_world_group",1,all2all_out_flag=True),"ms":lambda: npu_gmm_alltoallv(_ms_tensor(np.ones((2,8),np.float16)),_ms_tensor(np.ones((8,4),np.float16)),"hccl_world_group",1,out_flag=True)},
 {"id":"out_flag_false","torch":lambda: torch_npu.npu_gmm_alltoallv(_torch_tensor(np.ones((2,8),np.float16)),_torch_tensor(np.ones((8,4),np.float16)),"hccl_world_group",1,all2all_out_flag=False),"ms":lambda: npu_gmm_alltoallv(_ms_tensor(np.ones((2,8),np.float16)),_ms_tensor(np.ones((8,4),np.float16)),"hccl_world_group",1,out_flag=False)},
]


@pytest.mark.parametrize("case", CASES, ids=lambda c: c["id"])
def test_npu_gmm_alltoallv_matches_torch_npu(case):
    try:
        expected = case["torch"]()
        actual = case["ms"]()
