...
std::vector<ms::Tensor> npu_fusion_attention_v3(
  ...
  std::optional<ms::Tensor> dropout_mask_opt = std::nullopt;
  auto dropout_mask_value = ms::Tensor();
  ...
  auto runner = std::make_shared<ms::pynative::AclnnOpRunner>("FlashAttentionScoreV3");
  if (sink_opt.has_value()) {
    runner->SetLaunchFunc(LAUNCH_ACLNN_FUNC(aclnnFlashAttentionScoreV3, query, key, value, pse_opt, dropout_mask_opt,
                                            padding_mask_opt, atten_mask_opt, sink_opt, prefix, q_start_idx,
                                            kv_start_idx, scale, keep_prob, pre_tockens, next_tockens, head_num,
                                            input_layout, inner_precise, sparse_mode, 1, softmax_max, softmax_sum,
  ...
  runner->Run({query, key, value, pse_value, padding_mask_value, atten_mask_value, sink_value, dropout_mask_value},
              {attention, softmax_max, softmax_sum, softmax_out});
  return {attention, softmax_max, softmax_sum, softmax_out};
}

PYBIND11_MODULE(MS_EXTENSION_NAME, m) {
  m.def("npu_fusion_attention_v3", PYBOOST_CALLER(4, custom::npu_fusion_attention_v3));
}
}  // namespace custom
