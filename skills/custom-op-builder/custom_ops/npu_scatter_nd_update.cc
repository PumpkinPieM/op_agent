...
ms::Tensor npu_scatter_nd_update(const ms::Tensor &self, const ms::Tensor &indices, const ms::Tensor &updates) {
  auto out = self;
...
PYBIND11_MODULE(MS_EXTENSION_NAME, m) {
  m.def("npu_scatter_nd_update", PYBOOST_CALLER(1, custom::npu_scatter_nd_update));
}
}  // namespace custom
