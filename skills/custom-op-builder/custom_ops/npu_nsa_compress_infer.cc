...
ms::Tensor npu_nsa_compress_infer(const ms::Tensor & input, const ms::Tensor & weight, const ms::Tensor & slot_mapping, int64_t compress_block_size, int64_t compress_stride, int64_t page_block_size, const std::optional<ms::Tensor> & block_table_opt, const std::optional<std::vector<int64_t>> & actual_seq_len_opt, const ms::Tensor & cache) {
  auto out=ms::Tensor(cache.data_type(), cache.shape()); auto block_table=block_table_opt.value_or(ms::Tensor()); auto actual_seq_len=std::make_pair(actual_seq_len_opt.value_or(std::vector<int64_t>{}), true);
  auto runner = std::make_shared<ms::pynative::AclnnOpRunner>("NsaCompressWithCache");
  runner->SetLaunchFunc(LAUNCH_ACLNN_FUNC(aclnnNsaCompressWithCache, input, weight, slot_mapping, compress_block_size, compress_stride, page_block_size, block_table_opt, actual_seq_len, out));
