...
_custom_ops = ms.ops.CustomOpBuilder("custom_ops_npu_mm_reduce_scatter_base_test", [str(KERNEL_SOURCE)], backend="Ascend").load()
def npu_mm_reduce_scatter_base(*args):
    return _custom_ops.npu_mm_reduce_scatter_base(*args)
...
def _case():
    ...
    comm_mode_opt = None
    return (_pta(self), _pta(x2), hcom, world_size, reduce_op_opt, _pta(bias_opt), _pta(x1_scale_opt), _pta(x2_scale_opt), comm_turn, output_dtype_opt, comm_mode_opt), (_ms(self), _ms(x2), hcom, world_size, reduce_op_opt, _ms(bias_opt), _ms(x1_scale_opt), _ms(x2_scale_opt), comm_turn, output_dtype_opt, comm_mode_opt)
def _torch_reference(torch_args):
    required_count = 4
    keyword_names = ['reduce_op', 'bias', 'x1_scale', 'x2_scale', 'comm_turn', 'output_dtype', 'comm_mode']
    ...
    return torch_npu.npu_mm_reduce_scatter_base(*torch_args[:required_count], **kwargs)

def test_npu_mm_reduce_scatter_base_against_torch_npu_benchmark():
    assert hasattr(torch_npu, "npu_mm_reduce_scatter_base")
    torch_args, ms_args = _case()
    expected = _torch_reference(torch_args)
    actual = npu_mm_reduce_scatter_base(*ms_args)
