...
_custom_ops = ms.ops.CustomOpBuilder("custom_ops_npu_mla_prolog_v3_test", [str(KERNEL_SOURCE)], backend="Ascend").load()
...
def _torch_reference(torch_args):
    ...
    return torch_npu.npu_mla_prolog_v3(*torch_args[:required_count], **kwargs)

def test_npu_mla_prolog_v3_against_torch_npu_benchmark():
    assert hasattr(torch_npu, "npu_mla_prolog_v3")
