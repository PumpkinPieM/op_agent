...
KERNEL_SOURCE = Path(__file__).with_name("npu_mla_prolog_v3.cc")
...
_custom_ops = ms.ops.CustomOpBuilder(
    "custom_ops_npu_mla_prolog_v3_test",
    [str(KERNEL_SOURCE)],
    backend="Ascend",
).load()


def npu_mla_prolog_v3(*args):
    return _custom_ops.npu_mla_prolog_v3(*args)
...
def _case(bs_merged, query_norm_flag):
    torch.manual_seed(2 + int(bs_merged) + int(query_norm_flag) * 10)
...
@pytest.mark.parametrize("query_norm_flag", [False, True])
def test_npu_mla_prolog_v3_against_torch_npu_benchmark(bs_merged, query_norm_flag):
    if not hasattr(torch_npu, "npu_mla_prolog_v3"):
        pytest.skip("torch_npu.npu_mla_prolog_v3 is not available")
    torch_args, kwargs, ms_args = _case(bs_merged, query_norm_flag)
    expected = torch_npu.npu_mla_prolog_v3(*torch_args, **kwargs)
    actual = npu_mla_prolog_v3(*ms_args)
