...
_custom_ops = ms.ops.CustomOpBuilder("custom_ops_npu_nsa_compress_attention_test", [str(KERNEL_SOURCE)], backend="Ascend").load()
...
def _case():
    ...
    return (_pta(query), _pta(key), _pta(value), scale_value, head_num, compress_block_size, compress_stride, select_block_size, select_block_count, _pta(topk_mask_opt), _pta(atten_mask_opt), actual_seq_qlen_opt, actual_cmp_seq_kvlen_opt, actual_sel_seq_kvlen_opt), (_ms(query), _ms(key), _ms(value), scale_value, head_num, compress_block_size, compress_stride, select_block_size, select_block_count, _ms(topk_mask_opt), _ms(atten_mask_opt), actual_seq_qlen_opt, actual_cmp_seq_kvlen_opt, actual_sel_seq_kvlen_opt)
def _torch_reference(torch_args):
    required_count = 9
    keyword_names = ['topk_mask', 'atten_mask', 'actual_seq_qlen', 'actual_cmp_seq_kvlen', 'actual_sel_seq_kvlen']
    ...
    return torch_npu.npu_nsa_compress_attention(*torch_args[:required_count], **kwargs)

def test_npu_nsa_compress_attention_against_torch_npu_benchmark():
    assert hasattr(torch_npu, "npu_nsa_compress_attention")
