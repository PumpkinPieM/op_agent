...
std::vector<ms::Tensor> npu_moe_token_unpermute_with_routing_map(const ms::Tensor &permuted_tokens, const ms::Tensor &sorted_indices, const std::vector<int64_t> &restore_shape, const std::optional<ms::Tensor> &probs_opt = std::nullopt, const std::optional<ms::Tensor> &routing_map_opt = std::nullopt, bool drop_and_pad = false) {
...
PYBIND11_MODULE(MS_EXTENSION_NAME, m) {
  m.def("npu_moe_token_unpermute_with_routing_map", PYBOOST_CALLER(1, custom::npu_moe_token_unpermute_with_routing_map));
}
}  // namespace custom
