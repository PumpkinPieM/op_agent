...
Covers both Graph mode (ops.Custom) and PyNative mode (CustomOpBuilder)
via @pytest.mark.parametrize('context_mode', CONTEXT_MODES).

Run:
    pytest -sv test_ms_vs_pta_custom_aclnn_sparse_lightning.py
    pytest -sv test_ms_vs_pta_custom_aclnn_sparse_lightning.py::test_bsnd_with_rope
"""
...
from custom_op import npu_sparse_lightning_indexer_grad_kl_loss
...
def _gen_tnd_flattened_inputs_np(  # pylint: disable=too-many-arguments,too-many-locals
    ...
    sparse = np.zeros((t1, nidx2, top_k), dtype=np.int32)
    t_idx = 0
    for b_idx, _ in enumerate(seqlens_q):
        sq = seqlens_q[b_idx]
        sk = seqlens_kv[b_idx]
        for s_idx in range(sq):
            s2_real = int(sk - sq + s_idx + 1)
...
# =========================================================================
def _run_ms_graph(inputs_np, scale_value, layout, sparse_mode,
                  pre_tokens, next_tokens,
                  seqlens_q=None, seqlens_kv=None, use_bf16=False):
    """Run MS ops.Custom Cell in GRAPH_MODE, return 4-tuple of numpy."""
...
# =========================================================================
def _run_ms_pynative(inputs_np, scale_value, layout, sparse_mode,
                     pre_tokens, next_tokens,
                     seqlens_q=None, seqlens_kv=None, use_bf16=False):
    """Run MS CustomOpBuilder op in PYNATIVE_MODE, return 4-tuple of numpy."""
    ...
    results = npu_sparse_lightning_indexer_grad_kl_loss(
        ...
        scale_value,
        _to_ms(inputs_np["query_rope"]),
        _to_ms(inputs_np["key_rope"]),
        seqlens_q,
        seqlens_kv,
        layout,
        sparse_mode,
...
# =========================================================================
def _run_ms_dispatch(context_mode, inputs_np, scale_value, layout,
    ...
    return runner(
        inputs_np, scale_value, layout, sparse_mode,
        pre_tokens, next_tokens,
        seqlens_q=seqlens_q, seqlens_kv=seqlens_kv, use_bf16=use_bf16)
...
def test_bsnd_with_rope(dtype, context_mode):
    """
    ...
    pta_out = _run_pta(
        ...
        use_bf16=use_bf16)
    ms_out = _run_ms_dispatch(
        context_mode, inputs, scale_value=1.0, layout="BSND",
        sparse_mode=3, pre_tokens=65536, next_tokens=65536,
        use_bf16=use_bf16)
...
@pytest.mark.parametrize(
    ...
    ids=['nidx1_8', 'nidx1_16', 'nidx1_32', 'nidx1_64'],
)
def test_bsnd_nidx1_matrix(nidx1, seed, context_mode):
    """
    Feature: MS vs PTA bit-exact comparison
...
@pytest.mark.parametrize('context_mode', CONTEXT_MODES)
@pytest.mark.parametrize('dtype', ['fp16', 'bf16'])
def test_tnd_with_rope(dtype, context_mode):
    """
    Feature: MS vs PTA bit-exact comparison
    ...
    ms_out = _run_ms_dispatch(
        context_mode, inputs, scale_value=1.0, layout="TND",
        sparse_mode=3, pre_tokens=65536, next_tokens=65536,
        seqlens_q=[128], seqlens_kv=[128], use_bf16=use_bf16)
...
def test_tnd_scale_0p5_nidx1_matrix(nidx1, seed, context_mode):
    """
    Feature: MS vs PTA bit-exact comparison
    ...
    ms_out = _run_ms_dispatch(
        context_mode, inputs, scale_value=0.5, layout="TND",
        sparse_mode=3, pre_tokens=65536, next_tokens=65536,
        seqlens_q=[128], seqlens_kv=[128])
...
# =========================================================================
@pytest.mark.parametrize('context_mode', CONTEXT_MODES)
def test_tnd_scale_2p0(context_mode):
    """
    Feature: MS vs PTA bit-exact comparison
    ...
    pta_out = _run_pta(
        ...
        seqlens_q=[128], seqlens_kv=[128])
    ms_out = _run_ms_dispatch(
        context_mode, inputs, scale_value=2.0, layout="TND",
        sparse_mode=3, pre_tokens=65536, next_tokens=65536,
        seqlens_q=[128], seqlens_kv=[128])
...
# =========================================================================
@pytest.mark.parametrize('context_mode', CONTEXT_MODES)
def test_tnd_default_tokens(context_mode):
    """
    Feature: MS vs PTA bit-exact comparison
...
# =========================================================================
@pytest.mark.parametrize('context_mode', CONTEXT_MODES)
def test_bsnd_scale_0p5(context_mode):
    """
    Feature: MS vs PTA bit-exact comparison
    ...
    pta_out = _run_pta(
        ...
        sparse_mode=3, pre_tokens=65536, next_tokens=65536)
    ms_out = _run_ms_dispatch(
        context_mode, inputs, scale_value=0.5, layout="BSND",
        sparse_mode=3, pre_tokens=65536, next_tokens=65536)
...
# =========================================================================
@pytest.mark.parametrize('context_mode', CONTEXT_MODES)
def test_bsnd_scale_2p0(context_mode):
    """
    Feature: MS vs PTA bit-exact comparison
    ...
    pta_out = _run_pta(
        ...
        sparse_mode=3, pre_tokens=65536, next_tokens=65536)
    ms_out = _run_ms_dispatch(
        context_mode, inputs, scale_value=2.0, layout="BSND",
        sparse_mode=3, pre_tokens=65536, next_tokens=65536)
...
# =========================================================================
@pytest.mark.parametrize('context_mode', CONTEXT_MODES)
def test_bsnd_default_tokens(context_mode):
    """
    Feature: MS vs PTA bit-exact comparison
...
# =========================================================================
@pytest.mark.parametrize('context_mode', CONTEXT_MODES)
def test_tnd_pta_weights(context_mode):
    """
    Feature: MS vs PTA bit-exact comparison
...
# =========================================================================
@pytest.mark.parametrize('context_mode', CONTEXT_MODES)
def test_bsnd_pta_weights(context_mode):
    """
    Feature: MS vs PTA bit-exact comparison
...
def test_tnd_topk_doc_matrix(topk, seed, context_mode):
    """
    Feature: MS vs PTA bit-exact comparison
