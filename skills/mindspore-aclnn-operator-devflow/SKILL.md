...
ACLNN 算子开发遵循 **Pre + 11 步流程**，从前置分析到转测交付。
...
└───────────────────┘     └───────────────────┘     └───────────────────┘
           ...
           ▼
┌───────────────────┐     ┌───────────────────┐     ┌───────────────────┐
│ 9. 文档           │ ──▶ │ 10. 远程编译验证  │ ──▶ │ 11. 转测交付      │
│ EN + CN 一致      │     │ 推送→编译→轮询修复│     │ Feature + RFC/PR  │
└───────────────────┘     └───────────────────┘     └───────────────────┘
```
...
- [ ] **Feature 文档定稿** `🔒不可跳过`：补齐 §13（代码改动）、§14（验收报告）、更新 §3 任务状态
  - 即使 Step 9/10/11 被跳过或推迟，Feature 文档也必须在代码开发完成后补齐
- [ ] **[Step 10](workflows/10-remote-build.md)**：远程编译验证
  - 输入：已 commit 的代码变更 + 远程 Ascend 服务器信息（自动从 `servers.json` 读取）
  - 输出：编译成功 / 自动修复后编译成功 / 3 次重试后失败报告
  - 使用 `scripts/remote_deploy_build.py` 推送代码并触发编译
  - **编译失败自动重试**：抓取错误日志 → 定位修复代码 → 重新推送编译，最多 3 次
- [ ] **[Step 11](workflows/11-delivery.md)**：转测交付
  - 输入：全部代码/测试/文档 + **完整 Feature 文档** + 远程编译通过
...
| 无远程 Ascend 服务器 / 用户未提供连接信息 | 跳过 10(远程编译) | 用户自行在服务器上编译验证 |
...
>
> Feature 文档是算子评审和转测交付的必须产物，即使跳过 Step 9（文档）或推迟 Step 10/11，
> Feature 文档本身也必须生成。
...
- 远程部署编译脚本：`scripts/remote_deploy_build.py`（Step 10 使用）
- 服务器配置：`servers.json`（远程编译服务器 IP/账号/密码/目录/编译命令）
