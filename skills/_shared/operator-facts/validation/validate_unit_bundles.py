...
DEFAULT_BUNDLE_ROOT = ROOT_FROM_COMMON / "bundles" / "units"
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Validate refactored MindSpore unit bundles.")
    parser.add_argument("--bundle-root", type=Path, default=DEFAULT_BUNDLE_ROOT)
    parser.add_argument("--schema", type=Path, default=SCHEMA_DIR / "ms_unit_bundle.schema.json")
    ...
    if errors:
        print(f"validate_unit_bundles: FAILED ({len(errors)} issues)")
        for error in errors:
            print(f"- {error}")
        return 1

    print("validate_unit_bundles: OK")
